...
fn main() -> ! {
    ...
    loop {
        ...
        core::write!(&mut buf, "X{}", count).unwrap();
        count += 1;
        ...
        Text::with_baseline("String1", Point::zero(), text_style, Baseline::Top)
            .draw(&mut display)
            .unwrap();

        Text::with_baseline("String2", Point::new(0, 16), text_style, Baseline::Top)
            .draw(&mut display)
            .unwrap();
